...
export async function GET(req: NextRequest) {
  ...
  const impact = searchParams.get('impact') as BehavioralImpact | null;
  const impactRadius = searchParams.get('scope') as string | null;

  // semantic filters
  const q = searchParams.get('q');
  const relatedTo = searchParams.get('related-to') ? Number(searchParams.get('related-to')) : null;
  const repoId = searchParams.get('repoId') ? Number(searchParams.get('repoId')) : null;
